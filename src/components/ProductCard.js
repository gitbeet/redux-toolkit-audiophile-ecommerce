...
export default function ProductCard({ product, arrangement }) {
  ...
  useEffect(() => {
    import(
      `../assets/product-${product.slug}/mobile/image-category-page-preview.jpg`
    ).then((bg) => setMobileImage(bg.default));
    import(
      `../assets/product-${product.slug}/tablet/image-category-page-preview.jpg`
    ).then((bg) => setTabletImage(bg.default));
    import(
      `../assets/product-${product.slug}/desktop/image-category-page-preview.jpg`
    ).then((bg) => setDesktopImage(bg.default));
  }, []);
