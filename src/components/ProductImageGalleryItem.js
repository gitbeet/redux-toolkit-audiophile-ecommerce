...
export default function ProductImageGalleryItem({ images, area }) {
  ...
  const mobImage = images[1].mobile
    .split("/")
    .splice(2, images[1].mobile.length)
    .join("/");
  const tabImage = images[1].tablet
    .split("/")
    .splice(2, images[1].tablet.length)
    .join("/");
  const deskImage = images[1].desktop
    .split("/")
    .splice(2, images[1].desktop.length)
    .join("/");
